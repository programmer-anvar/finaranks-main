    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.key === 'ArrowDown') {
            ...
            if (selectedItem) {
                setSearchValue('');
                setItems([]);
                window.location.href = `/stock/${selectedItem.code}/summary`;
            }
                        ...
                        {items.slice(0, 5).map((item, index) => (
                            <li
                                ...
                                onClick={() => {
                                    setSearchValue('');
                                    setItems([]);
                                    window.location.href = `/stock/${item.code}/summary`;
                                }}
                            >
                                <div >
                ...
                <ul className="flex items-center gap-2">
                    {
                        STOCK_BADGES.map((el) => {
                            return (
                                <li key={el.id}>
                                    <Badge size="lg" onClick={() => router.push(`/stock/${el.slug}/summary`)} className='cursor-pointer' variant="outline">{el.label}</Badge>
                                </li>
                            )
                        })
                    }
