...
const SearchBar: React.FC = () => {
    ...
    /* ---------------------------------- */
    return (
        <div className="relative top-[130px] md:top-[500px]  max-w-[960px] mx-auto">
            <div className="relative   left-0 right-0 mx-auto z-10 search_form">
                <div className="search_form__bg" />
                <Card
                    variant="secondary"
                    className="border-none shadow-none bg-transparent! w-full p-1 md:p-6 h-full relative flex flex-col gap-3 md:gap-6"
                >
                    {/* Search Input */}
                            ...
                            {STOCK_BADGES?.map((el) => (
                                <li key={el.id} className="flex-shrink-0">
                                    <Badge
                                        ...
                                        onClick={() =>
                                            router.push(`/stock/${el.slug}/summary`)
                                        }
                                    >
                                        {el.label}
                                    </Badge>
    ...
    touch-pan-x w-[calc(100%-4rem)] no-scrollbar">
            ...
            {/* Dropdown */}
            {items.length > 0 && (
                <div className="absolute   top-19  md:top-28 w-[95%] left-1/2 -translate-x-1/2 rounded-2xl border border-white/10 bg-[#0b0324] shadow-xl z-50 overflow-hidden">
                    <ul className="divide-y divide-border/40">
                        {items.slice(0, 5).map((item) => (
                            <li key={item.code}>
                                <Link
                                    href={`/stock/${item.code}/summary`}
                                    onClick={() => {
                                        setSearchValue("");
                                        setItems([]);
                                    }}
                                    className="flex items-center gap-3 px-4 py-2 md:py-3 transition-colors hover:bg-purple-500/10 focus:bg-purple-500/10"
                                >
                                    {/* Logo */}
                                    <Avatar
                                        src={item.logo}
