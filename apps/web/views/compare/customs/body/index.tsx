    ...
    return (
        <div>
            <Card className="rounded-xl p-6">
                ...
                <div className="border border-[#ffffff1f] rounded-[14px] grid grid-cols-5 mt-5 h-[150px]!">
                    <div className='flex items-center justify-center p-[20px] border-r w-[250px]'>
                    ...
                    {/* Base Stock Card */}
                    <div className="flex items-center justify-center border-r px-[24px] w-[245px]">
                        <div className="statistic-section__info">
                            ...
                            <div className="flex items-center justify-between gap-[20px]">
                                ...
                                <div className={`statistic-section__incr whitespace-nowrap ${change < 0 ? '--decr' : '--incr'}`}>
                                    ...
                                    <span className="whitespace-nowrap">{change_p.toFixed(2)} %</span>
                                </div>
                            </div>
                    ...
                    {compareItems.length > 0 && (
                        compareItems.map(compSlug => {
                            ...
                            return (
                                <div key={compSlug} className="flex items-center justify-center border-r px-[24px] relative w-[240px]">
                                    <div className="statistic-section__info cursor-pointer" onClick={() => {
                                        ...
                                        <div className="flex items-center justify-between gap-[20px]">
                                            ...
                                            <div className={`statistic-section__incr whitespace-nowrap  ${compChange < 0 ? '--decr' : '--incr'}`}>
                                                <img className='statistic-section__red' src="/icons/arrowRed.svg" alt="" />
                                                <img className='statistic-section__green' src="/icons/arrowGreen.svg" alt="" />
                                                <span className="whitespace-nowrap">{compChangeP.toFixed(2)} %</span>
                                            </div>
                                        </div>
                                        <div className="statistic-section__subname">
                                            {compName}
                        ...
                        <Table >
                            <TableHeader className='rounded-t-md!'>
                                <TableRow className="border-b border-[#353945] hover:bg-(--main-color) rounded-t-md! bg-(--main-color)">
                                    <TableHead className="text-white font-bold uppercase text-sm border-r border-[#353945]   p-4 w-[250px]">
                                        Metrics
                                    </TableHead>
                            ...
                            <TableBody>
                                {metrics.map((metric: any) => (
                                    <TableRow key={metric.label} className="border-b border-[#353945]  hover:bg-purple-900/10">
                                        <TableCell className="font-medium text-white border-r border-[#353945] p-3 w-[240px]">{metric.label}</TableCell>
                                        <TableCell className="text-white text-center border-r border-[#353945]  w-[240px]">{get(scores, `${metric.value}`)}</TableCell>
                                        {compareItems.length > 0 && (
                                            compareItems.map(item => (
                                                <TableCell className="text-white text-center border-r border-[#353945] w-[240px]">{get(compareScores, `${item}.${metric.value}`)}</TableCell>
                                            ))
                                        )}
                                        {[...Array(3 - compareItems.length)].map((_, index) => (
                                            <TableCell key={`empty-cell-${metric.value}-${index} text-white text-center border-r border-[#353945] w-[240px]`} style={{ width: '20%' }}></TableCell>
                                        ))}
                                    </TableRow>
                                ))}
                            </TableBody>
