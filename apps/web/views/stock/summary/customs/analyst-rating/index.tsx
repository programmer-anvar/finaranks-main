...
const AnalystRating: React.FC<AnalystRatingProps> = memo(({ data, dictionary }) => {
    const dic = dictionary?.stock?.stockMain?.summaryTab?.analystRating;
    
    const LABELS = {
        strongBuy: dic?.strongBuyLabel || "Strong buy",
        buy: dic?.buyLabel || "Buy",
        hold: dic?.holdLabel || "Hold",
        sell: dic?.sellLabel || "Sell",
        strongSell: dic?.strongSellLabel || "Strong sell"
    };

    const COLORS = {
        [LABELS.strongBuy]: "#6366F1",
        [LABELS.buy]: "#14B8A6",
        [LABELS.hold]: "#FACC15",
        [LABELS.sell]: "#F97316",
        [LABELS.strongSell]: "#EF4444"
    };
    ...
    const chartData = monthlyData.map((item: any) => {
        ...
        return {
            name: item.month,
            [LABELS.strongBuy]: strongBuy > 0 ? Math.max(strongBuy, MIN_BAR_VALUE) : strongBuy,
            [LABELS.buy]: buy > 0 ? Math.max(buy, MIN_BAR_VALUE) : buy,
            [LABELS.hold]: hold > 0 ? Math.max(hold, MIN_BAR_VALUE) : hold,
            [LABELS.sell]: sell > 0 ? Math.max(sell, MIN_BAR_VALUE) : sell,
            [LABELS.strongSell]: strongSell > 0 ? Math.max(strongSell, MIN_BAR_VALUE) : strongSell,
    ...
    const barPadding = barCount === 1 ? 0.3 : 0.2;

    const keyMap: Record<string, string> = {
        [LABELS.strongBuy]: "_StrongBuy",
        [LABELS.buy]: "_Buy",
        [LABELS.hold]: "_Hold",
        [LABELS.sell]: "_Sell",
        [LABELS.strongSell]: "_StrongSell"
    };
    ...
    return (
        <Card className="space-y-4 rounded-xl p-4 md:p-6">
            <Typography variant="h4">{dic?.analystRatingTitle}</Typography>
            ...
            <div className="h-[345px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 20, right: 0, left: -30, bottom: 20 }} barSize={45}>
                        ...
                        {[LABELS.strongBuy, LABELS.buy, LABELS.hold, LABELS.sell, LABELS.strongSell].map((key, idx) => (
                            <Bar
                                key={key}
                                dataKey={key}
                                stackId="a"
                                ...
                                label={(props) => {
                                    ...
                                    const actualValue = chartData[index][keyMap[key]];
                                    return (
                                        <text
                                            x={x + width / 2}
                                            y={y + height / 2}
