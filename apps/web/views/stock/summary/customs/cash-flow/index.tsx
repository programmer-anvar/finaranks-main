...
const Cashflow = memo(({ data, dictionary }: CashflowProps) => {
    const dic = dictionary?.stock?.stockMain?.summaryTab?.cashflow;
    
    const LABELS = {
        operatingCashFlow: dic?.operatingCashFlow || "Operating Cash Flow",
        freeCashFlow: dic?.freeCashFlow || "Free Cash Flow",
        financingCashFlow: dic?.financingCashFlow || "Financing Cash Flow",
        investingCashFlow: dic?.investingCashFlow || "Investing Cash Flow"
    };

    const formatData = Object.entries(data?.cashFlow?.annual || {}).map(
        ([year, val]: any) => ({
            year,
            [LABELS.operatingCashFlow]: val.operatingCashFlow,
            [LABELS.freeCashFlow]: val.freeCashFlow,
            [LABELS.financingCashFlow]: val.financingCashFlow,
            [LABELS.investingCashFlow]: val.investingCashFlow,
        })
    )
    ...
    const chartValues = formatData.flatMap(d =>
        [
            d[LABELS.operatingCashFlow],
            d[LABELS.freeCashFlow],
            d[LABELS.financingCashFlow],
            d[LABELS.investingCashFlow],
        ]
    )
            ...
            <div className="h-[345px] w-full" >
                <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={formatData} margin={{ top: 0, right: 3, bottom: 20, left: -16 }}>
                        ...
                        <Bar dataKey={LABELS.operatingCashFlow} fill="var(--primary-graph-color)" radius={2.6} />
                        <Bar dataKey={LABELS.freeCashFlow} fill="var(--secondary-graph-color)" radius={2.6} />
                        <Bar dataKey={LABELS.financingCashFlow} fill="var(--tertiary-graph-color)" radius={2.6} />
                        <Bar dataKey={LABELS.investingCashFlow} fill="var(--quaternary-graph-color)" radius={2.6} />
                    </ComposedChart>
                </ResponsiveContainer>
            </div>
        </Card>
