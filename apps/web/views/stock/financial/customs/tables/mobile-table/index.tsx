...
// Cash Flow Schema - complete hierarchical structure
const CashFlowSchema = [
    ...
    { data: { name: 'Free cash flow', key: 'free_cash_flow' }, color: 'green' }
];

// Balance Sheet Schema - complete hierarchical structure
const BalanceSheetSchema = [
    ...
    { data: { name: 'Total liabilities & shareholders\' equities', key: 'total_liabilities_shrhldrs_equity' }, color: 'purple' },
...
const IncomeStatementSchema = [
    ...
    },
    {
        ...
        children: [
            { data: { name: "Depreciation and amortization", key: "dep_amort_exp_income_s" } },
    ...
    {
        data: { name: "Operating expenses (excl. COGS)", key: "operating_expenses" },
        children: [
            {
                data: { name: "Selling/general/admin expenses, total", key: "sell_gen_admin_exp_total" },
                children: [
                    { data: { name: "Research & development", key: "research_and_dev" } },
                    { data: { name: "Selling/general/admin expenses, other", key: "sell_gen_admin_exp_other" } }
                ]
            },
            { data: { name: "Other operating expenses, total", key: "other_oper_expense_total" } }
        ]
    },
    {
    ...
    {
        data: { name: "Non-operating income, total", key: "total_non_oper_income" },
        children: [
            {
                data: { name: "Interest expense, net of interest capitalized", key: "non_oper_interest_exp" },
                children: [
                    { data: { name: "Interest expense on debt", key: "interest_expense_on_debt" } },
                    { data: { name: "Interest capitalized", key: "interest_capitalized" } }
                ]
            },
            {
                data: { name: "Non-operating income, excl. interest expenses", key: "non_oper_income" },
                children: [
                    { data: { name: "Non-operating interest income", key: "non_oper_interest_income" } },
                    { data: { name: "Pretax equity in earnings", key: "pretax_equity_in_earnings" } },
                    { data: { name: "Miscellaneous non-operating expense", key: "other_income" } }
                ]
            },
            { data: { name: "Unusual income/expense", key: "unusual_expense_inc" } }
        ]
    },
    {
        data: { name: "Pretax income", key: "pretax_income" },
        color: 'purple',
        showGrowth: true
    },
    { data: { name: "Equity in earnings", key: "equity_in_earnings" } },
    { data: { name: "Taxes", key: "income_tax" } },
    { data: { name: "Non-controlling/minority interest", key: "minority_interest_exp" } },
    { data: { name: "After tax other income/expense", key: "after_tax_other_income" } },
    { data: { name: "Net income before discontinued operations", key: "net_income_bef_disc_oper" } },
    { data: { name: "Discontinued operations", key: "discontinued_operations" } },
    {
        data: { name: "Net income", key: "net_income" },
        color: 'green',
        showGrowth: true
    },
    { data: { name: "Dilution adjustment", key: "dilution_adjustment" } },
    { data: { name: "Preferred dividends", key: "preferred_dividends" } },
    { data: { name: "Diluted net income available to common stockholders", key: "diluted_net_income" } },
    { data: { name: "Basic earnings per share (Basic EPS)", key: "earnings_per_share_basic" } },
    { data: { name: "Diluted earnings per share (Diluted EPS)", key: "earnings_per_share_diluted" } },
    { data: { name: "Average basic shares outstanding", key: "basic_shares_outstanding" } },
    { data: { name: "Diluted shares outstanding", key: "diluted_shares_outstanding" } },
    { data: { name: "EBITDA", key: "ebitda" } },
    { data: { name: "EBIT", key: "ebit" } },
...
const MobileFinancialTable = ({ data, view, type = 'income', financialsNew }: any) => {
    ...
    const periods = financialsNew?.periods?.[view]?.fiscal_periods || [];

    // Show all periods from API
    const displayPeriods = periods;

    if (Object.keys(financialData).length > 0) {
        console.log('Sample Metric (total_revenue):', financialData.total_revenue);
    }

    // Use appropriate schema based on type
    const metricSchema = type === 'income'
    ...
    const formatValue = (value: any) => {
        ...
        return formatToMillion(value);
    };

    const calculateGrowth = (currentValue: any, previousValue: any) => {
    ...
    const getColorClass = (color: string) => {
        const colors = {
            blue: 'mobile-fin-table__row--blue',
            orange: 'mobile-fin-table__row--orange',
            purple: 'mobile-fin-table__row--purple',
            green: 'mobile-fin-table__row--green',
            default: ''
        };
        return colors[color as keyof typeof colors] || '';
    };

    // Flatten schema to flat list - no nesting/indentation
    const flattenSchema = (items: any) => {
        const result: any[] = [];

        const flatten = (items: any, parentName = '') => {
            items.forEach((item: any) => {
                ...
                if (item.children && item.children.length > 0) {
                    flatten(item.children, item.data.name);
                }
            });
        };
    ...
    const flatMetrics = flattenSchema(metricSchema);

    // Render flat rows - simple and clean
    const renderRows = () => {
        return flatMetrics
            .filter(item => !searchTerm || item.data.name.toLowerCase().includes(searchTerm.toLowerCase()))
            .map((item, idx) => {
                const rowClasses = [
                    'mobile-fin-table__row',
                    getColorClass(item.color),
                    item.isChild ? 'mobile-fin-table__row--child' : ''
                ].filter(Boolean).join(' ');

                return (
                    <tr key={`${item.data.key}-${idx}`} className={rowClasses}>
                        <td className="mobile-fin-table__cell mobile-fin-table__cell--sticky">
                            <div className="mobile-fin-table__label">
                                {item.isChild && <span className="mobile-fin-table__bullet">•</span>}
                                <span>{item.data.name}</span>
                            </div>
                        </td>
                        {displayPeriods.map((periodKey: any, periodIdx: number) => {
                            // Get the values array for this metric
                            const valuesArray = financialData[item.data.key] || [];

                            // Get value for this period by index
                            const value = valuesArray[periodIdx];
                            const prevValue = valuesArray[periodIdx - 1];

                            const growth = item.showGrowth && value !== null && value !== undefined && prevValue !== null && prevValue !== undefined
                                ? calculateGrowth(value, prevValue)
                                : null;

                            // Debug first item
                            if (idx === 0 && periodIdx === 0) {
                            ...
                            return (
                                <td key={periodIdx} className="mobile-fin-table__cell">
                                    <div className="mobile-fin-table__value">
                                        {formatValue(value)}
                                    </div>
                                </td>
                            );
    ...
    return (
        <div className="mobile-fin-table space-y-4">
            {/* Search Box */}
            <div>
                <SearchInput
            ...
            <div className="mobile-fin-table__container">
                <div className="mobile-fin-table__scroll">
                    <table className="mobile-fin-table__table">
                        {/* Header */}
                        <thead>
                            <tr>
                                ...
                                {displayPeriods.map((periodKey: any, idx: number) => (
                                    <th key={idx} className="mobile-fin-table__header-cell">
                                        <div className="mobile-fin-table__period">
                                            {periodKey}
                                        </div>
                                    </th>
                                ))}
...
export default MobileFinancialTable;
