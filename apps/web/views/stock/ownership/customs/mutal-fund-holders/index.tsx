...
const MutalFundHolders = ({ data, dictionary }: any) => {
    const funds = get(data, 'funds.data', []);
    const dic = dictionary?.stock?.stockMain?.ownershipTab?.topMutualFundHolders;
    return (
        <Card className='md:p-6 p-5 rounded-xl space-y-4'>
            <Typography variant="h2" className="text-[20px]!" weight="semibold">{dic?.topMutualFundHoldersTitle || "Top Mutual Fund Holders"}</Typography>
